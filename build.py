...
def remove_old_build():
    target_dir = Path(__file__).parent.joinpath("resources").joinpath("static")
...
def build_with_gox():
    ...
    try:
        ...
        return_code = run_command([
            ...
            "-arch", "amd64",
            "-ldflags", ldflags,
            "-output", "%s/aic-cli_<no value>_<no value>" % output_dir
        ])
        ...
        if return_code == 0:
            ...
            for file in os.listdir(output_dir):
                if file.startswith("aic-cli_windows") and not file.endswith(".exe"):
...
def build_go_app():
    ...
    try:
        ...
        # 设置输出文件名
        output_name = "aic-cli.exe" if os.name == 'nt' else "aibookmark"
